...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (!id) {
    console.warn('[api/incidents][POST status] missing id in params')
  ...
  const url = `${backendUrl}/api/incidents/${id}/status`
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.debug('[api/incidents][POST status] proxy ->', url, { body, outgoingHeaders })
    }
    ...
    console.error('[api/incidents][POST status] proxy error', e)
